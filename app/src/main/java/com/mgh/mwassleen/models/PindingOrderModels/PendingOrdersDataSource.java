...
public class PendingOrdersDataSource extends PageKeyedDataSource<Long , Datum> {
    ...
    public void loadInitial(@NonNull LoadInitialParams<Long> params, @NonNull final LoadInitialCallback<Long, Datum> callback) {
        ...
        RetroWeb.getClient().create(ServiceApi.class).onGetPendingOrders(globalPrefrencies.getLanguage(), FIRST_PAGE, "Bearer " + globalPrefrencies.getApi_token()).enqueue(new Callback<PindigModel>() {
            @Override
            public void onResponse(Call<PindigModel> call, Response<PindigModel> response) {
                PindigModel model = response.body();
                if (model!= null) {
            ...
            @Override
            public void onFailure(Call<PindigModel> call, Throwable t) {
    ...
    public void loadBefore(@NonNull final LoadParams<Long> params, @NonNull final LoadCallback<Long, Datum> callback) {
        globalPrefrencies = new GlobalPrefrencies(context);
        RetroWeb.getClient().create(ServiceApi.class).onGetPendingOrders(globalPrefrencies.getLanguage(), params.key, "Bearer " + globalPrefrencies.getApi_token()).enqueue(new Callback<PindigModel>() {
            @Override
            public void onResponse(Call<PindigModel> call, Response<PindigModel> response) {
                PindigModel model = response.body();
                if (model != null) {
...
//        globalPrefrencies = new GlobalPrefrencies(context);
//
//        RetroWeb.getClient().create(ServiceApi.class).onGetPendingOrders(globalPrefrencies.getLanguage(),"Bearer " + globalPrefrencies.getApi_token()).enqueue(new Callback<PindigModel>() {
//            @Override
//            public void onResponse(Call<PindigModel> call, Response<PindigModel> response) {
//                PindigModel model = response.body();
//                if (model != null) {
